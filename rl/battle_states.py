...
def battlefield_to_observation(battlefield: BattleField):
    ...
    obs.append(battlefield.action_count)
    obs.append(battlefield.tune.points)
    
    card_count = 0
    for card in battlefield.current_cards:
    ...
    for character in battlefield.red_team:
    ...
    for status in battlefield.blue_team[0].status:
        obs.append(status.id.value)
        obs.append(min(status.times_count, status.turn_count, 30))
        obs.append(status.value)
    ...
    if (len(obs) != 90):
        print(len(battlefield.current_cards))
        print(obs)
